...
export async function findClientPda(signer: PublicKey): Promise<[PublicKey, number]> {
...
export async function findTaskPda(signer: PublicKey, taskCounter: BN): Promise<[PublicKey, number]> {
  return PublicKey.findProgramAddress(
    [
      Buffer.from("TASK"),
      signer.toBuffer(),
...
export async function findEndpointNodePda(signer: PublicKey): Promise<[PublicKey, number]> {
  return PublicKey.findProgramAddress(
...
export async function findProviderNodePda(signer: PublicKey): Promise<[PublicKey, number]> {
  return PublicKey.findProgramAddress(
    [Buffer.from("PROVIDER_NODE"), signer.toBuffer()],
    PROGRAM_ID
  );
}

export async function findTokenVaultPda(): Promise<[PublicKey, number]> {
  return PublicKey.findProgramAddress(
...
export function createCreateNodeInstruction(
  ...
  const ipv4Buffer = Buffer.from(args.ipv4);
