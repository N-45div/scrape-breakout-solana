...
interface Task {
  ...
  downloadUrl?: string; // Added to store the off-chain download URL
}
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const storedTasks: TaskMetadata[] = JSON.parse(
        localStorage.getItem(`tasks_${keypair.publicKey.toString()}`) || "[]"
      );
      ...
      const [clientPda] = await findClientPda(keypair.publicKey);
      ...
      const combinedTasks = fetchedTasks.map((task) => {
        ...
        return {
          ...task,
          downloadUrl: metadata?.downloadUrl || task.ipfsHash, // Use off-chain downloadUrl if available
          url: metadata?.url || task.url, // Ensure URL matches what was submitted
          format: metadata?.format || task.format, // Ensure format matches what was submitted
        };
      });
