...
import { findProviderNodePda } from "../program/program";
...
function NodeSettingsPage({ walletAddress }: NodeSettingsPageProps) {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      // Call the backend to update the node settings
      const response = await fetch("http://localhost:3000/update-node", {
        ...
        body: JSON.stringify({
          ...
          ipv4: nodeData.ipv4,
          proxyPort: nodeData.proxyPort,
          clientPort: nodeData.clientPort,
          bandwidthLimit: newBandwidthLimitBytes.toString(),
        }),
      });
      ...
      setStatus(`Bandwidth updated successfully! New limit: ${newBandwidthLimitMB} MB/hour. Transaction: ${data.signature}`);
      setTimeout(() => navigate("/", { state: { updatedBandwidth: newBandwidthLimitMB } }), 2000);
    } catch (error) {
